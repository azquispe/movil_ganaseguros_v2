        ...
        TableRow(children: [
          TableCell(
              child: _SingleCard(
                  ...
                  text: loginProvider.datosPersonaModel.personaId != null
                      ? 'Mis pólizas'
                      : 'Consultar Póliza',
                  pageName: 'consulta_poliza_historico_page')),

          if(   loginProvider.datosPersonaModel.personaId != null)
            TableCell(
              child: _SingleCard(
              pathImg: "assets/icon/icon_facturas.jpg",
              color: colores.sec_negro_claro1,
              text: 'Mis facturas',
              pageName: 'lista_facturas_page')),
...
class _SingleCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final loginProvider = Provider.of<LoginProvider>(context);
    final consultaPolizaProvider =
        Provider.of<ConsultaPolizaProvider>(context, listen: false);
    final datosFacturaProvider =
        Provider.of<DatosFacturaProvider>(context, listen: false);
    DatosPersonaModel datosPersonaModel = loginProvider.datosPersonaModel;
    return GestureDetector(
        onTap: () async {
          if ((datosPersonaModel.personaId != null &&
                  datosPersonaModel.personaId! > 0) &&
              this.pageName == "consulta_poliza_historico_page") {
            consultaPolizaProvider.txtNroDocumentoController.text =
                datosPersonaModel.numeroDocumento.toString();
            consultaPolizaProvider.ciudadExpedidoId =
                datosPersonaModel.ciudadExpedidoId!;
            consultaPolizaProvider.txtComplementoController.text =
                datosPersonaModel.complemento.toString();
            consultaPolizaProvider.txtFechaNacimientoController.text =
                datosPersonaModel.fechaNacimiento.toString();
            ...
            if (lstPolizaModel != null && lstPolizaModel.length > 0) {
              ...
              Dialogos.dialogoInformativo(
                      ...
                      pDescripcion:
                          "No se ha encontrado Póliza \n Si desea consultar Póliza para otro asegurado ingrese como INVITADO  o creando una nueva CUENTA",
                      pContext: context,
                      pBoton: "Aceptar",
                      pTipoAlerta: AlertType.info)
                  .show();
            }
          } else if ((datosPersonaModel.personaId != null &&
                  datosPersonaModel.personaId! > 0) &&
              this.pageName == "lista_facturas_page") {
            List<DatosFacturaModel>? lstDatosfacturaModel = await showDialog(
              context: context,
              builder: (context) => FutureProgressDialog(
                  datosFacturaProvider.consultarDatosFactura(),
                  message: Text(
                    'Procesando...',
                    style: GoogleFonts.poppins(
                      textStyle: TextStyle(
            ...
            if (lstDatosfacturaModel != null &&
                lstDatosfacturaModel.length > 0) {
              Navigator.of(context).pushNamed('lista_facturas_page');
            } else {
              Dialogos.dialogoInformativo(
                      pTitulo: "Consulta de Facturas",
                      pDescripcion: "No cuenta con facturas",
                      pContext: context,
                      pBoton: "Aceptar",
                      pTipoAlerta: AlertType.info)
                  .show();
